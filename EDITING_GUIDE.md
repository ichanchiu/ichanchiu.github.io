...
網站首頁用簡短文字說明研究興趣；「研究／Research」頁則直接作為論文目錄，整合期刊論文、審查中論文與工作論文。國科會計畫只在首頁呈現。日常更新仍以 `_data` 內的資料檔為主，不需要在兩個頁面重複貼內容。
...
| `_data/grants.yml` | 國科會研究計畫（顯示於首頁） |
...
每篇論文都從同一縮排層級的 `- id:` 開始，更新後會自動出現在「研究／Research」頁與首頁的近期期刊論文區。新增論文時，最安全的方式是複製一篇現有論文的完整區塊，貼到檔案最上方或最下方，再逐欄修改。
...
這裡適合放工作論文、投稿中論文、修改後再審論文，以及仍在發展中的研究；它們會依狀態顯示在「研究／Research」頁。每筆資料使用 `section` 決定顯示區段：
...
8. **不要重複 `id`。** `_data/publications.yml` 與 `_data/working_papers.yml` 內的每篇論文都共用同一套網址識別碼，因此兩個檔案之間也不能重複。
